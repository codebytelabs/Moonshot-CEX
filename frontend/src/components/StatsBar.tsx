...
export default function StatsBar({ swarmStatus, portfolio }: Props) {
  ...
  const stats = [
    { label: "EQUITY", value: `$${equity.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: "text-cyan-400" },
    { label: "TOTAL PnL", value: `${totalPnl >= 0 ? "+" : ""}$${totalPnl.toFixed(2)}`, color: totalPnl >= 0 ? "text-green-400" : "text-red-400" },
    { label: "TODAY", value: `${dayPnl >= 0 ? "+" : ""}$${dayPnl.toFixed(2)}`, color: dayPnl >= 0 ? "text-green-400" : "text-red-400" },
    { label: "POSITIONS", value: `${openCount}`, color: "text-slate-300" },
    ...
    { label: "CYCLES", value: cycles.toLocaleString(), color: "text-slate-500" },
  ];
