        ...
        <CardContent>
          {events.length > 0 ? (
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {events.filter(e => e && e.type).map((event, i) => (
                <div
                ...
                >
                  <EventDot type={event.type as string} />
                  ...
                  <span className="text-xs text-muted-foreground whitespace-nowrap">
                    {event.timestamp ? timeAgo(event.timestamp as number) : ""}
                  </span>
                </div>
              ))}
            </div>
...
function formatEventDetail(event: EventData): string {
  try {
    ...
    return parts.join(" ") || (event.type as string) || "";
  } catch {
    return "";
  }
}
